...
Page({
  ...
  createActivity() {
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          const endDate = that.data.aDate || that.data.currentDate
          wx.request({
            url: `${baseUrl}/api/activity/add`,
            header: util.getAuthHeader(),
            method: "POST",
            data: {
              aName: that.getDefaultName(),
              aDesc: that.data.aDesc || "",
              aEndDate: String(endDate),
              aIsPublic: parseInt(that.data.aIsPublic) || 0,
              aVenue: that.data.aVenue || "",
              aLevel: that.data.aLevel || "",
              aDate: that.data.aDate ? String(that.data.aDate) : "",
              aTimeSlot: that.data.aTimeSlot || "",
              aCount: that.data.aCount || 4,
              aNeedApprove: that.data.aNeedApprove ? 1 : 0
            },
            success(res) {

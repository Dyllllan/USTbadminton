...
public class ActivityController {
    ...
    public RestfulResponse addActivity(HttpServletRequest request,
        ...
        log.info("[创建活动] aId={}", aId);

        activity.setAHolderId(uId);
        activity.setAId(aId);
        activity.setAQrcodePath("");
        activity.setStatus(1);
        log.info("[创建活动] 步骤2: 保存到MySQL");
        activityService.save(activity);
        log.info("[创建活动] 步骤3: 保存到Elasticsearch");
        try {
            elasticsearchOperations.save(activity);
        } catch (Exception e) {
            log.warn("Elasticsearch 索引保存失败(活动已存入数据库): {}", e.getMessage());
        }
        log.info("[创建活动] 完成");
